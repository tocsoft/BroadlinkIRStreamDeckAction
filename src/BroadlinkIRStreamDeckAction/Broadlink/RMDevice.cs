...
namespace Broadlink.NET
{
    ...
    public class RMDevice : BroadlinkDevice
    {
        ...
        public async Task<byte[]> LearnIRAsync()
        {
            var tcs = new TaskCompletionSource<byte[]>();
            var cancel = new CancellationTokenSource(10000);
            var handler = new EventHandler<byte[]>((o, pay) =>
            {
                tcs.TrySetResult(pay);
            });
            ...
            try
            {
                this.OnRawData += handler;
                await EnterIRLearningModeAsync();
                return await tcs.Task;
            }
            finally
            {
                this.OnRawData -= handler;
                await this.ExitIRLearningModeAsync();
            }
        }
        /// <summary>
